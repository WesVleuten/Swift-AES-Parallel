...
final public class AESCipher {
    ...
    private var blockMode = CBCBlockMode()
    ...
    func decrypt(bytes: [UInt8], callback: @escaping (([UInt8]) throws -> Void)) throws -> Void {
        if (bytes.count % AESCipher.blockSize) != 0 {
            throw Error.blockSizeExceeded
        }

        let blocks = bytes.chunks(size: AESCipher.blockSize)
        blockMode = CBCBlockMode()
        try blockMode.decrypt(blocks: blocks, iv: self.iv, cipherOperation: decrypt, callback: {(bytes: [UInt8]) in
            do {
                var result = try? PKCS7().remove(bytes: bytes, blockSize: AESCipher.blockSize)
                try? callback(result!)
            } catch {
                print("err", error);
            }
...
private struct CBCBlockMode {
    ...
    func decrypt(blocks: [[UInt8]], iv: [UInt8]?, cipherOperation: @escaping CipherOperationOnBlock, callback: @escaping (([UInt8]) -> ())) throws -> () {
        precondition(!blocks.isEmpty)
        guard let iv = iv else {
            throw BlockError.MissingInitializationVector
        }
        ...
        let blockperthread = blockcount / threads
        let blockperthreadrem = blockcount % threads
        let blockgrouper = BlockGrouper()
        
        let concurrentQueue = DispatchQueue(label: "com.queue.Concurrent", attributes: .concurrent)
        for t in 0..<threads {
            //calculate where we should start and end within the block array
            let blockRangeStart = (t * blockperthread)
            var blockRangeEnd = ((t+1) * blockperthread)
            if (t == threads-1) {
                //make sure remainder is included within the last blockgroup
                blockRangeEnd += blockperthreadrem
                print("last task initalized")
            }
            concurrentQueue.async() {
                //select all blocks for this task
                let subblockArray: [[UInt8]] = Array(blocks[blockRangeStart..<blockRangeEnd]);
                let block = BlockGroup(subblockArray)
                //count over all subblock values
                for (subsetindex, ciphertext) in block.rawvalues.enumerated() {
                    if let decrypted = cipherOperation(ciphertext) { // decrypt
                        if (subsetindex == 0) {
                            //set inital ciphertext
                            block.firstCiphertext = decrypted
                        } else {
                            //add decrypted to block
                            block.decrypted.append(contentsOf: xor(block.lastCiphertext!, decrypted))
                        }
                    } else {
                        print("this should not happend")
                    }
                    //set last ciphertext
                    block.lastCiphertext = ciphertext
                }
                DispatchQueue.main.async {
                    blockgrouper.groups[t] = block;

                    print("\(t) completed")
                    if (blockgrouper.groups.count == threads) {
                        
                        print("task complete");

                        var lastgroup = blockgrouper.groups[0]
                        //bridge between initial vector and frist group's first ciphertext
                        out.append(contentsOf: xor(iv, lastgroup!.firstCiphertext!))
                        //add conect of first group
                        out.append(contentsOf: lastgroup!.decrypted)
                        
                        for b in 1..<threads {
                            let g = blockgrouper.groups[b]
                            //bridge between lastgroup and newgroup
                            out.append(contentsOf: xor(lastgroup!.lastCiphertext!, g!.firstCiphertext!))
                            //add content of newgroup
                            out.append(contentsOf: g!.decrypted)
                            lastgroup = g
                        }
...
private class BlockGrouper {
    
    init() {
        
    }
    
    public var groups: [Int: BlockGroup] = [:]
...
private class BlockGroup {
    ...
    public let rawvalues: [[UInt8]]
    public var lastCiphertext: [UInt8]? = nil
    public var firstCiphertext: [UInt8]? = nil
    public var decrypted: [UInt8] = [UInt8]()
